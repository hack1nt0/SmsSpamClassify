 ...
 ************************************/
#ifndef CPPJIEBA_MIGPSEGMENT_H
#define CPPJIEBA_MIGPSEGMENT_H
...
namespace CppJieba
{
    ...
    class MIGPSegment: public SegmentBase
    {
        protected:
            Trie* _trie;

        public:
            MIGPSegment(){_setInitFlag(false);};
            explicit MIGPSegment(const string& dictPath)
            {
        ...
        public:
            bool init(const string& dictPath)
            {
                ...
                LogInfo("MIGPSegment init(%s) ok", dictPath.c_str());
